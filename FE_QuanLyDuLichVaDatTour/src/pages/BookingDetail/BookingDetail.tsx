            ...
            <Card className="customer-booking-card" variant="borderless">
              <Space direction="vertical" size={14} className="customer-booking-card-stack">
                ...
                <Descriptions bordered column={2} size="middle">
                  <Descriptions.Item label="Ngày đặt">{formatNgay(bookingQuery.data.ngayDat)}</Descriptions.Item>
                  <Descriptions.Item label="Đợt tour">{bookingQuery.data.maDotTour}</Descriptions.Item>
                  <Descriptions.Item label="Khởi hành">{formatNgay(bookingQuery.data.ngayKhoiHanh)}</Descriptions.Item>
                  <Descriptions.Item label="Kết thúc">{formatNgay(bookingQuery.data.ngayKetThuc)}</Descriptions.Item>
                  ...
                  <Descriptions.Item label="Tổng tiền">{formatTien(bookingQuery.data.tongTien)}</Descriptions.Item>
                  <Descriptions.Item label="Đã thanh toán">{formatTien(bookingQuery.data.soTienDaThanhToan)}</Descriptions.Item>
              ...
              {!paymentQuery.isLoading && !paymentQuery.isError ? (
                <List
                  ...
                  renderItem={(payment) => (
                    <List.Item>
                      <Space direction="vertical" size={2}>
                        <Text strong>{formatTien(payment.soTien)}</Text>
